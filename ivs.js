...
Object.keys(DataDict).forEach((p) => {
...
function executeIVCalc(name) {
  var name = name || document.getElementById('ivPokemon').value;
  if (!DataDict[name]) return;
  ...
  const link = Number(ivLink.value);
  const energy = +$('#ivEnergy').val() || getRadioValue('ivEnergy');

  const [min, max] = calculateIVs(stats, DataDict[name], link, energy);
  ...
  for (let i = 0; i < 4; i++) {
    if (min[i] === -1000 || max[i] === 1000) {
      ...
      total += min[i];
      out += `${labels[i]}: ${min[i]} (${Math.trunc((min[i] * 100) / 31)}%)\n`;
    } else {
      total += max[i];
      out += `${labels[i]}: ${min[i]}-${max[i]} (${Math.trunc((max[i] * 100) / 31)}%)\n`;
    }
  }
  out += `Total: ${Math.trunc((total * 100) / 31 / 4)}%`;
  ivResult.textContent = out.trim();
}

function executeStatCalc() {
  const name = document.getElementById('statPokemon').value;
  if (!DataDict[name]) return;

  const ivs = [
    Number(statIVHP.value),
    Number(statIVAtk.value),
    Number(statIVDef.value),
    Number(statIVSpe.value),
  ];

  const link = Number(statLink.value);
  const energy = getRadioValue('statEnergy');

  const stats = calculateStats(ivs, DataDict[name], link, energy);
  const labels = ['HP', 'Atk', 'Def', 'Spe'];

  let out = '';
  for (let i = 0; i < 4; i++) {
    out += `${labels[i]}: ${stats[i]}\n`;
  }
  statResult.textContent = out.trim();
}
